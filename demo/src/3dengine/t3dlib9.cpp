...
void Draw_Triangle_2DZB_32(POLYF4DV2_PTR face,   // ptr to face
                           UCHAR *_dest_buffer,   // pointer to video buffer
                           int mem_pitch,         // bytes per line, 320, 640 etc.
                           UCHAR *_zbuffer,       // pointer to z-buffer
                           int zpitch)            // bytes per line of zbuffer
...
void Draw_Textured_TriangleZB32(POLYF4DV2_PTR face,  // ptr to face
                                UCHAR *_dest_buffer,  // pointer to video buffer
                                int mem_pitch,        // bytes per line, 320, 640 etc.
                                UCHAR *_zbuffer,       // pointer to z-buffer
                                int zpitch)          // bytes per line of zbuffer
...
} // end Draw_Textured_TriangleZB32
...
void Draw_Textured_TriangleFSZB32(POLYF4DV2_PTR face, // ptr to face
                                  UCHAR *_dest_buffer,  // pointer to video buffer
                                  int mem_pitch,        // bytes per line, 320, 640 etc.
                                  UCHAR *_zbuffer,       // pointer to z-buffer
                                  int zpitch)          // bytes per line of zbuffer
...
} // end Draw_Textured_TriangleFSZB32
...
void Draw_Gouraud_TriangleZB32(POLYF4DV2_PTR face,   // ptr to face
                               UCHAR *_dest_buffer,   // pointer to video buffer
                               int mem_pitch,         // bytes per line, 320, 640 etc.
                               UCHAR *_zbuffer,       // pointer to z-buffer
                               int zpitch)            // bytes per line of zbuffer
                ...
                (x2 < min_clip_x) || (x2 > max_clip_x))
            {
                ...
                for (yi = ystart; yi<=yend; yi++)
                {
                    ...
                    for (xi=xstart; xi<=xend; xi++)
                    {
                        ...
                        if (zi < (int)z_ptr[xi])
                        {
                            // write textel assume 5.6.5 - nope!
                            // screen_ptr[xi] = ((ui >> (FIXP16_SHIFT+3)) << 11) + 
                            //     ((vi >> (FIXP16_SHIFT+2)) << 5) + 
                            //     (wi >> (FIXP16_SHIFT+3));

                            screen_ptr[xi] = ((ui >> (FIXP16_SHIFT)) << 16) + 
                                ((vi >> (FIXP16_SHIFT)) << 8) + 
                                (wi >> (FIXP16_SHIFT)); 
...
void Draw_RENDERLIST4DV2_SolidZB32(RENDERLIST4DV2_PTR rend_list, 
                                   UCHAR *video_buffer, 
                                   int lpitch,
                                   UCHAR *zbuffer,
                                   int zpitch)
{
    ...
    for (int poly=0; poly < rend_list->num_polys; poly++)
    {
        ...
        if (rend_list->poly_ptrs[poly]->attr & POLY4DV2_ATTR_SHADE_MODE_TEXTURE)
        {
            ...
            {
                // draw the textured triangle as emissive
                Draw_Textured_TriangleZB32(&face, video_buffer, lpitch,zbuffer,zpitch);
            } // end if
            else
            {
                // draw as flat shaded
                face.lit_color[0] = rend_list->poly_ptrs[poly]->lit_color[0];
                Draw_Textured_TriangleFSZB32(&face, video_buffer, lpitch,zbuffer,zpitch);
            } // end else
                ...
                (rend_list->poly_ptrs[poly]->attr & POLY4DV2_ATTR_SHADE_MODE_CONSTANT) )
            {
                ...
                // draw the triangle with basic flat rasterizer
                Draw_Triangle_2DZB_32(&face, video_buffer, lpitch,zbuffer,zpitch);
                ...
                if (rend_list->poly_ptrs[poly]->attr & POLY4DV2_ATTR_SHADE_MODE_GOURAUD)
                {
                    ...
                    // draw the gouraud shaded triangle
                    Draw_Gouraud_TriangleZB32(&face, video_buffer, lpitch,zbuffer,zpitch);
                } // end if gouraud

    } // end for poly

} // end Draw_RENDERLIST4DV2_SolidZB32
