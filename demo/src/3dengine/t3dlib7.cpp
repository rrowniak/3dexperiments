...
char texture_path[80] = "./media/"; // root path to ALL textures, make current directory for now
        ...
        case TRANSFORM_LOCAL_TO_TRANS:
            {
                ...
                for (int vertex=0; vertex < obj->total_vertices; vertex++)
                {
                    // POINT4D presuslt; // hold result of each transformation
                           ...
                           int max_lights)      // maximum lights in list
{
    ...
    for (int poly=0; poly < obj->num_polys; poly++)
    {
        ...
        if (curr_poly->attr & POLY4DV2_ATTR_SHADE_MODE_FLAT)
        {
            ...
            r_base = palette[curr_poly->color].r;
            g_base = palette[curr_poly->color].g;
            b_base = palette[curr_poly->color].b;
...
int Light_RENDERLIST4DV2_World32(RENDERLIST4DV2_PTR rend_list,  // list to process
                                 CAM4DV1_PTR /*cam*/,     // camera position
                                 LIGHTV1_PTR lights,  // light list (might have more than one)
                                 int max_lights)      // maximum lights in list
{
    ...
    for (int poly=0; poly < rend_list->num_polys; poly++)
    {
        ...
        if (curr_poly->attr & POLY4DV2_ATTR_SHADE_MODE_FLAT)
        {
            ...
            // step 1: extract the base color out in RGB mode
            // assume 565 format
            _RGBFROM32BIT(curr_poly->color, &r_base, &g_base, &b_base);
            ...
            // write the color over current color
            curr_poly->lit_color[0] = _RGB32Bit(r_sum, g_sum, b_sum);
            ...
            if (curr_poly->attr & POLY4DV2_ATTR_SHADE_MODE_GOURAUD) /////////////////////////////////
            {
                ...
                // write the colors
                curr_poly->lit_color[0] = _RGB32Bit(r_sum0, g_sum0, b_sum0);
                curr_poly->lit_color[1] = _RGB32Bit(r_sum1, g_sum1, b_sum1);
                curr_poly->lit_color[2] = _RGB32Bit(r_sum2, g_sum2, b_sum2);
...
} // end Light_RENDERLIST4DV2_World32
...
int Load_OBJECT4DV2_COB(OBJECT4DV2_PTR obj,   // pointer to object
                        const char *filename_,       // filename of Caligari COB file
                        ...
                        // and perform transforms
{
    ...
    if (!parser.Open(filename_))
    {
        Write_Error("Couldn't open .COB file %s.", filename_);
        return(0);
    } // end if
    ...
    while(1)
    {
        ...
        {
            Write_Error("Image 'name' not found in .COB file %s.", filename_);
            return(0);
        } // end if
    ...
    while(1)
    {
        ...
        if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
        {
            Write_Error("Center not found in .COB file %s.", filename_);
            return(0);
        } // end if
    ...
    while(1)
    {
        ...
        if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
        {
            Write_Error("Transform not found in .COB file %s.", filename_);
            return(0);
        } // end if
    ...
    while(1)
    {
        ...
        if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
        {
            Write_Error("'World Vertices' line not found in .COB file %s.", filename_);
            return(0);
        } // end if
    ...
    if (!Init_OBJECT4DV2(obj,   // object to allocate
        ...
        obj->num_frames))
    {
        Write_Error("\nASC file error with file %s (can't allocate memory).",filename_);
    } // end if
    ...
    for (int vertex = 0; vertex < obj->num_vertices; vertex++)
    {
        ...
        while(1)
        {
            ...
            if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
            {
                Write_Error("\nVertex list ended abruptly! in .COB file %s.", filename_);
                return(0);
            } // end if
    ...
    while(1)
    {
        ...
        if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
        {
            Write_Error("'Texture Vertices' line not found in .COB file %s.", filename_);
            return(0);
        } // end if
    ...
    for (tvertex = 0; tvertex < num_texture_vertices; tvertex++)
    {
        ...
        while(1)
        {
            ...
            if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
            {
                Write_Error("\nTexture Vertex list ended abruptly! in .COB file %s.", filename_);
                return(0);
            } // end if
    ...
    while(1)
    {
        ...
        if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
        {
            Write_Error("\n'Faces' line not found in .COB file %s.", filename_);
            return(0);
        } // end if
        ...
        {
            Write_Error("\nCOB Reader found face list in .COB file %s.", filename_);
    ...
    for (int poly=0; poly < obj->num_polys; poly++)
    {
        ...
        while(1)
        {
            ...
            if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
            {
                Write_Error("\nface list ended abruptly! in .COB file %s.", filename_);
                return(0);
            } // end if
            ...
            if (parser.Pattern_Match(parser.buffer, "['Face'] ['verts'] [i] ['flags'] [i] ['mat'] [i]"))
            {
                ...
                if (parser.pints[0]!=3)
                {
                    Write_Error("\nface not a triangle! in .COB file %s.", filename_);
                    return(0);
                } // end if
    ...
    for (curr_material = 0; curr_material < num_materials_object; curr_material++)
    {
        ...
        while(1)
        {
            ...
            if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
            {
                Write_Error("\nmaterial list ended abruptly! in .COB file %s.", filename_);
                return(0);
            } // end if
            ...
            if (parser.Pattern_Match(parser.buffer, "['mat#'] [i]") )
            {
                ...
                while(1)
                {
                    ...
                    if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
                    {
                        Write_Error("\nRGB color ended abruptly! in .COB file %s.", filename_);
                        return(0);
                    } // end if
                ...
                while(1)
                {
                    ...
                    if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
                    {
                        Write_Error("\nmaterial properties ended abruptly! in .COB file %s.", filename_);
                        return(0);
                    } // end if
                ...
                while(1)
                {
                    ...
                    if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
                    {
                        Write_Error("\nshader class ended abruptly! in .COB file %s.", filename_);
                        return(0);
                    } // end if
                ...
                while(1)
                {
                    ...
                    if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
                    {
                        Write_Error("\nshader name ended abruptly! in .COB file %s.", filename_);
                        return(0);
                    } // end if
                    ...
                    if (parser.Pattern_Match(parser.buffer, "['Shader'] ['name:'] ['texture'] ['map']") )
                    {
                        ...
                        while(1)
                        {
                            ...
                            if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
                            {
                                Write_Error("\ncouldnt find texture name! in .COB file %s.", filename_);
                                return(0);
                            } // end if
                            ...
                            if (parser.Pattern_Match(parser.buffer, "['file'] ['name:'] ['string']") )
                            {
                                ...
                                if (!obj->texture)
                                {
                                    ...
                                    char* end = path_filename + strlen(path_filename) - 1;
                                    while(*end == ' ') { *end = 0; --end; }

                                    // buffer now holds final texture path and file name
                                    // load the bitmap(8/16 bit)
                                    if (!Load_Bitmap_File(&bitmap32bit, path_filename)) {
                                        Write_Error("Can't load BMP file '%s'", path_filename);
                                        return 0;
                                    }

                                    // create a proper size and bitdepth bitmap
                                    Create_Bitmap(obj->texture,0,0,
                                        bitmap32bit.bitmapinfoheader.biWidth,
                                        bitmap32bit.bitmapinfoheader.biHeight,
                                        bitmap32bit.bitmapinfoheader.biBitCount);

                                    // load the bitmap image (later make this 8/16 bit)
                                    if (obj->texture->bpp == 32) {
                                        Load_Image_Bitmap32(obj->texture, &bitmap32bit,0,0,BITMAP_EXTRACT_MODE_ABS);
                                    } else if (obj->texture->bpp == 16) {
                                        Load_Image_Bitmap16(obj->texture, &bitmap32bit,0,0,BITMAP_EXTRACT_MODE_ABS);
                                    } else {
                                        Load_Image_Bitmap(obj->texture, &bitmap32bit,0,0,BITMAP_EXTRACT_MODE_ABS);
                                    } // end else 8 bit
                ...
                while(1)
                {
                    ...
                    if (!parser.Getline(PARSER_STRIP_EMPTY_LINES | PARSER_STRIP_WS_ENDS))
                    {
                        Write_Error("\nshader reflectance class not found in .COB file %s.", filename_);
                        return(0);
                    } // end if
    ...
    for (int curr_poly = 0; curr_poly < obj->num_polys; curr_poly++)
    {
        ...
        if (screen_bpp == 32)
        {
            ...
            if (materials[ poly_material[curr_poly] ].attr & MATV1_ATTR_SHADE_MODE_TEXTURE)
                obj->plist[curr_poly].color = _RGB32Bit(255,255,255);
            else
                obj->plist[curr_poly].color = _RGB32Bit(materials[ poly_material[curr_poly] ].color.r, 
    ...
    for (int poly=0; poly < obj->num_polys; poly++)
    {
        ...
        while(1)
        {
            ...
            if (parser.Pattern_Match(parser.buffer, "[i] [i] [i]"))
            {
                ...
                if (screen_bpp == 32) {
                    // cool, 32 bit mode
                    SET_BIT(obj->plist[poly].attr,POLY4DV2_ATTR_RGB32);
                    obj->plist[poly].color = _RGB32Bit(r, g, b);
                    Write_Error("\nPolygon 32-bit");
                } else if (screen_bpp==16) {
                    // cool, 16 bit mode
                        ...
                        int /*vertex_flags*/)       // vertex flags, used to override
{
    ...
    for (int poly=0; poly < obj->num_polys; poly++)
    {
        ...
        if ((poly_surface_desc & PLX_COLOR_MODE_RGB_FLAG)) 
        {
            ...
            obj->plist[poly].color = _RGB32Bit(red*16, green*16, blue*16);
...
void Draw_RENDERLIST4DV2_Wire32(RENDERLIST4DV2_PTR rend_list, 
                                UCHAR *video_buffer, int lpitch)
{
    // this function "executes" the render list or in other words
    // draws all the faces in the list in wire frame 16bit mode
    ...
    for (int poly=0; poly < rend_list->num_polys; poly++)
    {
        ...
        // by 2D initialization, so line clipper will clip all polys out
        // of the 2D screen/window boundary
        Draw_Clip_Line32(rend_list->poly_ptrs[poly]->tvlist[0].x, 
            rend_list->poly_ptrs[poly]->tvlist[0].y,
            rend_list->poly_ptrs[poly]->tvlist[1].x, 
            rend_list->poly_ptrs[poly]->tvlist[1].y,
            rend_list->poly_ptrs[poly]->lit_color[0],
        ...
        Draw_Clip_Line32(rend_list->poly_ptrs[poly]->tvlist[1].x, 
            rend_list->poly_ptrs[poly]->tvlist[1].y,
            rend_list->poly_ptrs[poly]->tvlist[2].x, 
            rend_list->poly_ptrs[poly]->tvlist[2].y,
            rend_list->poly_ptrs[poly]->lit_color[0],
        ...
        Draw_Clip_Line32(rend_list->poly_ptrs[poly]->tvlist[2].x, 
            rend_list->poly_ptrs[poly]->tvlist[2].y,
            rend_list->poly_ptrs[poly]->tvlist[0].x, 
            rend_list->poly_ptrs[poly]->tvlist[0].y,
            rend_list->poly_ptrs[poly]->lit_color[0],
...
} // end Draw_RENDERLIST4DV2_Wire32
...
void Draw_RENDERLIST4DV2_Solid32(RENDERLIST4DV2_PTR rend_list, 
                                 UCHAR *video_buffer, 
                                 int lpitch)
{
    // 16-bit version
    ...
    for (int poly=0; poly < rend_list->num_polys; poly++)
    {
        ...
        if (rend_list->poly_ptrs[poly]->attr & POLY4DV2_ATTR_SHADE_MODE_TEXTURE)
        {
            ...
            {
                // draw the textured triangle as emissive
                Draw_Textured_Triangle32(&face, video_buffer, lpitch);
            } // end if
            else
            {
                // draw as flat shaded
                face.lit_color[0] = rend_list->poly_ptrs[poly]->lit_color[0];
                Draw_Textured_TriangleFS32(&face, video_buffer, lpitch);
            } // end else
            ...
            {
                // draw the triangle with basic flat rasterizer
                Draw_Triangle_2D2_32(rend_list->poly_ptrs[poly]->tvlist[0].x, rend_list->poly_ptrs[poly]->tvlist[0].y,
                    rend_list->poly_ptrs[poly]->tvlist[1].x, rend_list->poly_ptrs[poly]->tvlist[1].y,
                    rend_list->poly_ptrs[poly]->tvlist[2].x, rend_list->poly_ptrs[poly]->tvlist[2].y,
                    rend_list->poly_ptrs[poly]->lit_color[0], video_buffer, lpitch);
            ...
            if (rend_list->poly_ptrs[poly]->attr & POLY4DV2_ATTR_SHADE_MODE_GOURAUD)
            {
                ...
                // draw the gouraud shaded triangle
                Draw_Gouraud_Triangle32(&face, video_buffer, lpitch);
            } // end if gouraud

    } // end for poly

} // end Draw_RENDERLIST4DV2_Solid32
                              ...
                              int mem_pitch)        // bytes per line, 320, 640 etc.
{
    ...
    // for lighting
    r_base = palette[face->lit_color[0]].r;
    g_base = palette[face->lit_color[0]].g;
    b_base = palette[face->lit_color[0]].b;
...
void Draw_Textured_Triangle32(POLYF4DV2_PTR face,   // ptr to face
                              UCHAR *_dest_buffer,   // pointer to video buffer
                              int mem_pitch)        // bytes per line, 320, 640 etc.
{
    // this function draws a textured triangle in 16-bit mode
    ...
    uint32_t *screen_ptr  = NULL,
        // *screen_line = NULL,
        *textmap     = NULL,
        *dest_buffer = (uint32_t *)_dest_buffer;
    ...
    // extract texture map
    textmap = (uint32_t *)face->texture->buffer;
...
void Draw_Textured_TriangleFS32(POLYF4DV2_PTR face,   // ptr to face
                                UCHAR *_dest_buffer,   // pointer to video buffer
                                int mem_pitch)        // bytes per line, 320, 640 etc.
{
    // this function draws a textured triangle in 16-bit mode with flat shading
    ...
    { 
        ...
        // extract base color of lit poly, so we can modulate texture a bit
        // for lighting
        _RGBFROM32BIT(face->lit_color[0], &r_base, &g_base, &b_base);
                ...
                (x2 < min_clip_x) || (x2 > max_clip_x))
            {
                ...
                for (yi = ystart; yi<=yend; yi++)
                {
                    ...
                    for (xi=xstart; xi<=xend; xi++)
                    {
                        ...
                        // and they all cancel out for the most part, but we will need logical anding, we will do
                        // it later when we optimize more...
                        // screen_ptr[xi] = ((b_textel >> 5) + ((g_textel >> 6) << 8) + ((r_textel >> 5) << 16));
                        // screen_ptr[xi] = _RGB32Bit(255, (g_textel >> 5), (b_textel >> 5));
                        screen_ptr[xi] = 
                                ((b_textel >> 8) 
                                + ((g_textel >> 8) << 8) 
                                + ((r_textel >> 8) << 16));
            ...
            else
            {
                ...
                for (yi = ystart; yi<=yend; yi++)
                {
                    ...
                    for (xi=xstart; xi<=xend; xi++)
                    {
                        ...
                        // screen_ptr[xi] = _RGB32Bit(255, 0, 0);
                    ...
                    (x2 < min_clip_x) || (x2 > max_clip_x))
                {
                    ...
                    for (yi = ystart; yi<=yend; yi++)
                    {
                        ...
                        for (xi=xstart; xi<=xend; xi++)
                        {
                            ...
                            // screen_ptr[xi] = _RGB32Bit(0, 255, 0);
...
} // end Draw_Textured_TriangleFS32
...
void Draw_Top_Tri2_32(float x1, float y1, 
                      float x2, float /*y2*/, 
                      float x3, float y3,
                      int color, 
                      UCHAR *_dest_buffer, int mempitch)
{
    ...
    // cast dest buffer to ushort
    uint32_t *dest_buffer = (uint32_t *)_dest_buffer;
...
void Draw_Bottom_Tri2_32(float x1, float y1, 
                         float x2, float /*y2*/, 
                         float x3, float y3,
                         int color,
                         UCHAR *_dest_buffer, int mempitch)
...
void Draw_Triangle_2D2_32(float x1,float y1,
                          float x2,float y2,
                          float x3,float y3,
                          int color,
                          UCHAR *dest_buffer, int mempitch)
{
    ...
    if (FCMP(y1,y2))
    {
        Draw_Top_Tri2_32(x1,y1,x2,y2,x3,y3,color, dest_buffer, mempitch);
    } // end if
    else
        if (FCMP(y2,y3))
        {
            Draw_Bottom_Tri2_32(x1,y1,x2,y2,x3,y3,color, dest_buffer, mempitch);
        } // end if bottom is flat
        else
        {
            // general triangle that's needs to be broken up along long edge
            ...
            // draw each sub-triangle
            Draw_Bottom_Tri2_32(x1,y1,new_x,y2,x2,y2,color, dest_buffer, mempitch);
            Draw_Top_Tri2_32(x2,y2,new_x,y2,x3,y3,color, dest_buffer, mempitch);
        } // end else

} // end Draw_Triangle_2D2_32
...
int Load_Bitmap_PCX_File(BITMAP_FILE_PTR bitmap, char *filename)
{
    ...
    for (index=0; index < 256; index++)
    {
        ...
        bitmap->palette[index].r   = (unsigned char)getc(fp);

        // get the green component
        bitmap->palette[index].g = (unsigned char)getc(fp);

        // get the blue component
        bitmap->palette[index].b  = (unsigned char)getc(fp);
...
///////////////////////////////////////////////////////////////////////////////
void Draw_Gouraud_Triangle32(POLYF4DV2_PTR face,   // ptr to face
                             UCHAR *_dest_buffer, // pointer to video buffer
                             int mem_pitch)       // bytes per line, 320, 640 etc.
{
    // this function draws a gouraud shaded polygon, based on the affine texture mapper, instead
    ...
    { 
        ...
        // 32 bit format
        _RGBFROM32BIT(face->lit_color[v0], &r_base0, &g_base0, &b_base0);
        _RGBFROM32BIT(face->lit_color[v1], &r_base1, &g_base1, &b_base1);
        _RGBFROM32BIT(face->lit_color[v2], &r_base2, &g_base2, &b_base2);
...
int RGB_12_8_Lighting_Table_Builder(LPPALETTEENTRY src_palette,       // source palette
                                    UCHAR rgblookup_[4096][256])  // lookup table
{
    ...
    for (int rgbindex = 0; rgbindex < 4096; rgbindex++)
    {
        ...
        for (int color_index = 0; color_index < 256; color_index++)
        {
            ...
            r = (int)(( (float)r * (float)src_palette[color_index].r)   / 15);
            g = (int)(( (float)g * (float)src_palette[color_index].g) / 15);
            b = (int)(( (float)b * (float)src_palette[color_index].b)  / 15);

            // now scan palette to find this color
            for (int color_scan = 0; color_scan < 256; color_scan++)
            {
                // compute distance to color from target
                long delta_red   = abs(src_palette[color_scan].r   - r);
                long delta_green = abs(src_palette[color_scan].g - g);
                long delta_blue  = abs(src_palette[color_scan].b  - b);
            ...
            rgblookup_[rgbindex][color_index] = curr_index;   
