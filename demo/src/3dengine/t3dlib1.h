...
typedef struct POLYGON2D_TYP
{
    ...
    long int color;      // could be index or PALETTENTRY
...
int Draw_BOB32(BOB_PTR bob, GfxSystem& engine);
...
int Draw_Clip_Line32(int x0,int y0, int x1, int y1, int color,UCHAR *dest_buffer, int lpitch);
...
int Draw_Line32(int xo, int yo, int x1,int y1, int color,UCHAR *vb_start,int lpitch);
...
int Draw_Bitmap32(BITMAP_IMAGE_PTR source_bitmap,UCHAR *dest_buffer, int lpitch, int transparent);
...
int Load_Image_Bitmap32(BITMAP_IMAGE_PTR image,BITMAP_FILE_PTR bitmap,int cx,int cy,int mode);               
...
void Draw_Top_Tri32(int x1,int y1,int x2,int y2, int x3,int y3,int color,UCHAR *dest_buffer, int mempitch);
void Draw_Bottom_Tri32(int x1,int y1, int x2,int y2, int x3,int y3,int color,UCHAR *dest_buffer, int mempitch);
...
void Draw_Triangle_2D32(int x1,int y1,int x2,int y2,int x3,int y3,
                        int color,UCHAR *dest_buffer, int mempitch);
...
extern BITMAP_FILE          bitmap32bit;          // a 32 bit bitmap file
