                                ...
                                int poly_attr)          // the shading attributes we would like
{
    ...
    {
        rgbwhite = _RGB32Bit(255,255,255);
    } // end else
    ...
    memset(&bitmap32bit, 0, sizeof(BITMAP_FILE));
    ...
    if ( (poly_attr & POLY4DV2_ATTR_SHADE_MODE_TEXTURE) && texture_map_file)
    {
        ...
        Load_Bitmap_File(&bitmap32bit, texture_map_file);
        ...
        // load the bitmap image (later make this 8/16/32 bit)
        ...
        col_tstep = (float)(bitmap32bit.bitmapinfoheader.biWidth-1)/(float)(columns - 1);
        row_tstep = (float)(bitmap32bit.bitmapinfoheader.biHeight-1)/(float)(rows - 1);
    ...
    uint32_t* buff = (uint32_t*)height_bitmap.buffer;
    for (int curr_row = 0; curr_row < rows; curr_row++)
    {
        for (int curr_col = 0; curr_col < columns; curr_col++)
        {
            ...
            auto pixel = buff[curr_col + (curr_row * columns)];
            pixel = pixel >> 14;
            obj->vlist_local[vertex].y = vscale*((float)pixel) / 255;
