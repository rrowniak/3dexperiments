...
BITMAP_FILE          bitmap32bit;            // a 32 bit bitmap file
...
int screen_width    = 0,            // width of screen
screen_height   = 0,           // height of screen
screen_bpp      = 0,              // bits per pixel
...

{
    ...
    for (index=0; index<MAX_BOB_FRAMES; index++)
        bob->images[index] = Texture(bob->width + bob->width_fill, bob->height);
               ...
               GfxSystem& engine)
{
    ...
    SDL_Rect dest_rect,   // the destination rectangle
        source_rect; // the source rectangle                             
    ...
    engine.RenderCopy(bob->images[bob->curr_frame], source_rect, dest_rect);
    // blt to destination surface
    // if (FAILED(dest->Blt(&dest_rect, bob->images[bob->curr_frame],
    //     &source_rect,(DDBLT_WAIT | DDBLT_KEYSRC),
    //     NULL)))
...
int Load_Frame_BOB32(BOB_PTR bob, // bob to load with data
                     ...
                     // cx,cy are absolute coords
{
    // this function extracts a 32-bit bitmap out of a 32-bit bitmap file

    // is this a valid bob
    if (!bob)
        return(0);

    uint32_t *source_ptr,   // working pointers
        *dest_ptr;
    ...
    // extract bitmap data
    source_ptr = (uint32_t *)bitmap->buffer 
        + cy*bitmap->bitmapinfoheader.biWidth+cx;

    // assign a pointer to the memory surface for manipulation
    bob->images[frame] = Texture(bob->width, bob->height);

    bob->images[frame].LockTexture();
    dest_ptr = bob->images[frame].Buffer();

    // iterate thru each scanline and copy bitmap
    for (int index_y=0; index_y<bob->height; index_y++) {
        // copy next line of data to destination
        memcpy(dest_ptr, source_ptr,(bob->width*4));

        // advance pointers
        dest_ptr   += (bob->images[frame].Pitch() >> 2); 
        source_ptr += bitmap->bitmapinfoheader.biWidth;
    } // end for index_y

    // unlock the surface 
     
    bob->images[frame].UnlockTexture();
                ...
                UCHAR *vb_start, int lpitch) // video buffer and memory pitch
{
    ...
    // pre-compute first pixel address in video buffer based on 16bit data
    uint32_t *vb_start2 = (uint32_t *)vb_start + x0 + y0*lpitch_2;
...
int Load_Palette_From_File(char *filename, LPPALETTEENTRY /*palette*/)
{
    ...
    for (int index=0; index<MAX_COLORS_PALETTE; index++)
    {
        ...
        // fscanf(fp_file,"%d %d %d %d",&palette[index].r,
        //     &palette[index].g,
        //     &palette[index].b,                                
    ...
    LPDIRECTDRAWSURFACE7 /*lpdds*/)
{
    ...
    // // set the colors for the text up
    // SetTextColor(xdc,RGB(palette[color].r,palette[color].g,palette[color].b) );
...
{
    // this function is used to intialize a bitmap, 8, 16 or 32 bit
    // allocate the memory
...
int Draw_Bitmap32(BITMAP_IMAGE_PTR source_bitmap,
    UCHAR *dest_buffer, int lpitch, int transparent)
{
    // this function draws the bitmap onto the destination memory surface
    // if transparent is 1 then color 0 (8bit) or 0.0.0 (16bit) will be transparent
    // note this function does NOT clip, so be carefull!!!
    ...
    uint32_t *dest_addr,   // starting address of bitmap in destination
        *source_addr; // starting adddress of bitmap data in source

    uint32_t pixel;        // used to hold pixel value

    int index,           // looping vars
        pixel_x,
        lpitch_2 = lpitch >> 2; // lpitch in uint terms

    // compute starting destination address
    dest_addr = ((uint32_t *)dest_buffer) 
        + source_bitmap->y*lpitch_2 + source_bitmap->x;

    // compute the starting source address
    source_addr = (uint32_t *)source_bitmap->buffer;
    ...
    else
    {
        ...
        int source_bytes_per_line = source_bitmap->width*4;
                        ...
                        // cx,cy are absolute coords
{
    ...
    // assign a pointer to the bimap image
    dest_ptr = (uint32_t *)image->buffer;
...
int Load_Bitmap_File16(BITMAP_FILE_PTR bitmap, const char *filename)
{
    ...
    if (bitmap->bitmapinfoheader.biBitCount == 8) {
        file.read((char*)&bitmap->palette, MAX_COLORS_PALETTE * sizeof(PALETTEENTRY));

        // now set all the flags in the palette correctly and fix the reversed 
        // BGR RGBQUAD data format
        for (int index=0; index < MAX_COLORS_PALETTE; index++) {
            // reverse the red and green fields
            int temp_color = bitmap->palette[index].r;
            bitmap->palette[index].r  = bitmap->palette[index].b;
            bitmap->palette[index].b = temp_color;
        }
    }

    // finally the image data itself
    file.seekg(-(int)(bitmap->bitmapinfoheader.biSizeImage), file.end);
    ...
    if (bitmap->bitmapinfoheader.biBitCount==8 || bitmap->bitmapinfoheader.biBitCount==16) {
        ...
        if (!(bitmap->buffer = (UCHAR *)malloc(
            2 * bitmap->bitmapinfoheader.biWidth * bitmap->bitmapinfoheader.biHeight))) {
        ...
        file.read((char*)temp_buffer, bitmap->bitmapinfoheader.biSizeImage);

        // now convert each 24 bit RGB value into a 16 bit value
        for (int index = 0; 
            index < (int)(bitmap->bitmapinfoheader.biWidth * bitmap->bitmapinfoheader.biHeight);
            index++) {
...
} // end Load_Bitmap_File16

///////////////////////////////////////////////////////////

// Assuming 32-bit buffer 
int Load_Bitmap_File(BITMAP_FILE_PTR bitmap, const char *filename)
{
    SDL_Surface* img = IMG_Load(filename);

    if (!img) {
        Write_Error("IMG_Load failed - %s\n",  IMG_GetError());
        return 0;
    }

    SDL_Surface* data = SDL_ConvertSurfaceFormat(img, SDL_PIXELFORMAT_ABGR32, 0);
    SDL_FreeSurface(img);

    if (bitmap->buffer) free(bitmap->buffer);
    bitmap->buffer = (UCHAR*)malloc(data->w * data->h * 4);
    if (!bitmap->buffer) return 0;
    // copy bits
    uint32_t* ptr = (uint32_t*)bitmap->buffer;
    for (int i = 0; i < data->h; ++i) {
        // memcpy(&ptr[i * data->w], (uint8_t*)data->pixels + i * data->pitch, data->pitch);
        for (int j = 0; j < data->w; ++j) {
            uint32_t* pixel = (uint32_t*)((uint8_t*)data->pixels + i * data->pitch);
            pixel += j;
            uint8_t r, g, b;
            SDL_GetRGB(*pixel, data->format, &r, &g, &b);
            ptr[i * data->w + j] = _RGB32Bit(r, g, b);
        }
    }

    bitmap->bitmapinfoheader.biWidth = data->w;
    bitmap->bitmapinfoheader.biHeight = data->h;
    ...
    case SCREEN_DARKNESS:
        {
            ...
            for (index=0; index<80; index++)
            {
                ...
                for (pal_reg=1; pal_reg<MAX_COLORS_PALETTE; pal_reg++)
                {
                    ...
                    if (color.r > 4) color.r-=3;
                    else
                        color.r = 0;

                    if (color.g > 4) color.g-=3;
                    else
                        color.g = 0;

                    if (color.b  > 4) color.b-=3;
                    else
                        color.b = 0;
...
void Draw_Top_Tri32(int x1,int y1, 
                    int x2,int /*y2*/, 
                    int x3,int y3,
                    int color, 
                    UCHAR *_dest_buffer, int mempitch)
{
    ...
    else
    {
        ...
        for (temp_y=y1; temp_y<=y3; temp_y++,dest_addr+=mempitch)
        {
            ...
            // draw the line
            Mem_Set_QUAD(dest_addr+(unsigned int)(left),color,(unsigned int)(right-left+1));
...
void Draw_Bottom_Tri32(int x1,int y1, 
                       int x2,int /*y2*/, 
                       int x3,int y3,
                       int color,
                       UCHAR *_dest_buffer, int mempitch)
...
void Draw_Triangle_2D32(int x1,int y1,
                        int x2,int y2,
                        int x3,int y3,
                        int color,
                        UCHAR *dest_buffer, int mempitch)
{
    ...
    if (y1==y2)
    {
        Draw_Top_Tri32(x1,y1,x2,y2,x3,y3,color, dest_buffer, mempitch);
    } // end if
    else
        if (y2==y3)
        {
            Draw_Bottom_Tri32(x1,y1,x2,y2,x3,y3,color, dest_buffer, mempitch);
        } // end if bottom is flat
        else
        {
            // general triangle that's needs to be broken up along long edge
            ...
            // draw each sub-triangle
            Draw_Bottom_Tri32(x1,y1,new_x,y2,x2,y2,color, dest_buffer, mempitch);
            Draw_Top_Tri32(x2,y2,new_x,y2,x3,y3,color, dest_buffer, mempitch);

        } // end else

} // end Draw_Triangle_2D32
