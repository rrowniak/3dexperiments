...
public:
    ...
    int SetComment(const char *string);
...
#define _RGBFROM32BIT(RGB, r,g,b) { *r = ( ((RGB) >> 16) & 0xff); *g = (((RGB) >> 8) & 0xff); *b = ((RGB) & 0xff); }
...
void Draw_RENDERLIST4DV1_Solid32(RENDERLIST4DV1_PTR rend_list, UCHAR *video_buffer, int lpitch);


int Convert_Bitmap_8_16(BITMAP_FILE_PTR bitmap);

int StripChars(const char *string_in, char *string_out, 
    const char *strip_chars, int case_on=1);

int ReplaceChars(const char *string_in, char *string_out, const char *replace_chars, char rep_char, int case_on=1);
...
extern LIGHTV1 glights[MAX_LIGHTS];  // lights in system
extern int gnum_lights;              // current number of lights
