...
void Draw_RENDERLIST4DV1_Solid32(RENDERLIST4DV1_PTR rend_list, 
                                 UCHAR *video_buffer, int lpitch)
{
    // this function "executes" the render list or in other words
    // draws all the faces in the list in wire frame 16bit mode
    ...
    for (int poly=0; poly < rend_list->num_polys; poly++)
    {
        ...
        // draw the triangle
        Draw_Triangle_2D32(rend_list->poly_ptrs[poly]->tvlist[0].x, rend_list->poly_ptrs[poly]->tvlist[0].y,
            rend_list->poly_ptrs[poly]->tvlist[1].x, rend_list->poly_ptrs[poly]->tvlist[1].y,
            rend_list->poly_ptrs[poly]->tvlist[2].x, rend_list->poly_ptrs[poly]->tvlist[2].y,
            rend_list->poly_ptrs[poly]->color, video_buffer, lpitch);
...
} // end Draw_RENDERLIST4DV1_Solid32
...
int CPARSERV1::Open(const char *filename)
{
#ifdef PARSER_DEBUG_ON
    printf("\nEntering Open().");
#endif
...
int CPARSERV1::SetComment(const char *string)
{
    // sets the comment string
...
int CPARSERV1::Pattern_Match(const char *string, const char *pattern_, ...)
{
    ...
    char buff[PARSER_BUFFER_SIZE]; // working buffer
    char pattern[PARSER_BUFFER_SIZE * 2];

    // a little error testing
    if ( (!string || strlen(string)==0) || (!pattern_ || strlen(pattern_)==0) )
        return(0);
    ...
    while(1)
    {
        ...
        // end of line?
        if ((size_t)tok_start >= strlen(pattern))
            break;    
            ...
            case PATTERN_TOKEN_STRING: // ascii string varying length
                {
                        ...
                        pattern[tok_start+2] == '<' )
                    {
                        ...
                        memcpy(buff, &pattern[tok_start+3], (tok_end - tok_start) );
                        buff[tok_end - tok_start] = 0;
                        ...
                        token_numeric[num_tokens]  = atoi(buff);
        ...
        case PATTERN_STATE_NEXT:
            {
                ...
                {
                    // get next token
                    if (tok_end >= (int)strlen(buff))
                        return(0);

                    tok_start = tok_end;
                    while(isspace(buff[tok_start])) tok_start++;
                    tok_end = tok_start;

                    while(!isspace(buff[tok_end])
                        && (size_t)tok_end < strlen(buff))
                        tok_end++;

                    // copy token
                    memcpy(token, &buff[tok_start], tok_end - tok_start);
        ...
        case PATTERN_STATE_STRING:
            {
                ...
                case '=':
                    {
                        // we need exactly
                        if (strlen(token) == (size_t)token_numeric[curr_tok])
                        {
...
int ReplaceChars(const char *string_in, char *string_out, const char *replace_chars, char rep_char, int case_on)
{
...
int StripChars(const char *string_in, char *string_out, 
    const char *strip_chars, int case_on)
{
...
int IsInt(const char *istring)
{
    ...
    if (strlen(istring) == (size_t)(string - istring))
        return(atoi(istring));
    else
        return(INT_MIN);
...
float IsFloat(const char *fstring)
{
    ...
    // the string better be the same size as the other one
    if (strlen(fstring) == (size_t)(string - fstring))
        return(atof(fstring));
    else
        return(FLT_MIN);
...
int Convert_Bitmap_8_16(BITMAP_FILE_PTR bitmap)
{
    ...
    for (int pixel = 0; pixel < bwidth*bheight; pixel++)
    {
        ...
        buffer16[pixel] = RGB16Bit(bitmap->palette[cindex].r, 
            bitmap->palette[cindex].g,
            bitmap->palette[cindex].b);
    } // end for
...
int RGBto8BitIndex(UCHAR r, UCHAR g, UCHAR b, LPPALETTEENTRY _palette, int flush_cache=0)
{
...
#define COLOR_CACHE_SIZE 16 // 16 entries should do for now
    ...
    for (int color_index = 0; color_index < 256; color_index++)
    {
        ...
        long delta_red   = abs(_palette[color_index].r   - r);
        long delta_green = abs(_palette[color_index].g - g);
        long delta_blue  = abs(_palette[color_index].b  - b);
    ...
    if (rgb_format==DD_PIXEL_FORMAT565)
    {
        ...
        for (int rgbindex = 0; rgbindex < 65536; rgbindex++)
        {
            ...
            for (int color_index = 0; color_index < 256; color_index++)
            {
                ...
                // compute distance to color from target
                long delta_red   = abs(src_palette[color_index].r   - r);
                long delta_green = abs(src_palette[color_index].g - g);
                long delta_blue  = abs(src_palette[color_index].b  - b);
                long error = (delta_red*delta_red) + (delta_green*delta_green) + (delta_blue*delta_blue);
            ...
            _rgblookup[rgbindex] = curr_index;             
...
int RGB_16_8_Indexed_Intensity_Table_Builder(LPPALETTEENTRY src_palette,  // source palette
                                             UCHAR _rgbilookup[256][256],  // lookup table
                                             int intensity_normalization)
{
    ...
    for (int col_index = 0; col_index < 256; col_index++)
    {
        ...
        ri = src_palette[col_index].r;
        gi = src_palette[col_index].g;
        bi = src_palette[col_index].b;
        ...
        for (int intensity_index = 0; intensity_index < 256; intensity_index++)
        {
            ...
            {
                // compute distance to color from target
                long delta_red   = abs(src_palette[color_index].r   - rw);
                long delta_green = abs(src_palette[color_index].g - gw);
                long delta_blue  = abs(src_palette[color_index].b  - bw);
                long error = (delta_red*delta_red) + (delta_green*delta_green) + (delta_blue*delta_blue);
            ...
            // best match has been found, enter it into table
            _rgbilookup[col_index][intensity_index] = curr_index;             
